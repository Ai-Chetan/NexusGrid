document.addEventListener('DOMContentLoaded', function() {
    ...
    const elements = {
        ...
        submitRequestBtn: document.getElementById('submitRequest'),
    ...
    if (elements.resetFilters) elements.resetFilters.addEventListener('click', resetFiltersFunc);
    ...
    function applyFiltersFunc() {
        ...
        if (elements.sortBy.value !== 'newest') params.set('sort', elements.sortBy.value);
        if (elements.timeFilter.value === 'custom') {
            if (elements.startDate.value) params.set('start', elements.startDate.value);
    ...
    setupAutocomplete(elements.labLocationInput, elements.labSuggestions, '/resources/labs-autocomplete/');
    ...
    if (elements.submitRequestBtn) {
        elements.submitRequestBtn.addEventListener('click', function () {
            if (!elements.newRequestForm.checkValidity()) {
                elements.newRequestForm.reportValidity();
                return;
            }
            const systemName = elements.systemNameInput.value.trim();
            const labLocation = elements.labLocationInput.value.trim();
            const resourceName = elements.resourceNameInput.value.trim();
            const description = elements.resourceDescriptionInput.value.trim();
    ...
    // Intercept status update form
    document.querySelectorAll('form[action*="update-resource-status"]').forEach(form => {
        form.addEventListener('submit', function(e) {
            const status = form.querySelector('select[name="status"]').value;
            if (status === 'Fulfilled') {
                e.preventDefault();
                const modalEl = document.getElementById('provisionSummaryModal');
                const modal = new bootstrap.Modal(modalEl);
                document.getElementById('provisionSummary').value = '';
                const summaryForm = document.getElementById('provisionSummaryForm');
                summaryForm.onsubmit = function(ev) {
                    ev.preventDefault();
                    const summary = document.getElementById('provisionSummary').value.trim();
                    if (!summary) {
                        document.getElementById('provisionSummary').focus();
                        return;
                    }
                    let input = form.querySelector('input[name="provision_summary"]');
                    if (!input) {
                        input = document.createElement('input');
                        input.type = 'hidden';
                        input.name = 'provision_summary';
