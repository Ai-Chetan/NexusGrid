...
:: Step 1: Send immediate offline signal to server
echo [1/4] Sending offline status signal to backend...
if exist "%SCRIPT_DIR%send_offline.bat" (
    call "%SCRIPT_DIR%send_offline.bat" >nul 2>&1
)
...
:: Step 4: Remove Startup folder entry
echo [4/4] Removing Startup folder fallback entries...
...
echo   - Offline signal sent to server : YES
echo =======================================================
echo.
